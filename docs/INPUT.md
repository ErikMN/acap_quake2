...
The WebSocket server accepts binary messages only. Protocol version 1 uses the first byte for the protocol version and the second byte for the message type. Multi-byte integers are little-endian.
...
Key identifiers are defined by `enum acap_key_code` in `src/input/input_queue.h`. They represent physical browser controls and are translated to Yamagi key values only on the Yamagi client thread.
